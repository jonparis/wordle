...
from wordleTools import WordleTools, KNOWLEDGE
...
class CLISolver(Solver):
    def make_a_guess(self):
        if self.game.god_mode:
            return input(
                f"THE SOLUTION IS {self.game._target_word}. You have as many guesses left as you want.").strip().lower()

        return input(
            f"{self.game.MAX_NUM_GUESSES - self.game.num_guesses_taken} guesses left. Please make a guess....").strip().lower()
    ...
    def make_a_guess(self):
        ...
        for word in filtered_words:
            for c in word:
                ...
                frequency_map[c] = count_map[c] / denominator if denominator > 0.0 else 0.0
...
class JPSolver(Solver):
    ...
    def on_guess_processed(self, guess):
        ...
        for i in range(len(guess.guess)):
            ...
            if guess_result == WordGuessCharPositionalResult.CORRECT:
                ...
                if c not in knowledge[str(i)][KNOWLEDGE.NOT_IN_POSITION]:
    ...
    def make_a_guess(self):
        new_guess = WordleTools.get_suggestion(self.knowledge, WORDS)
        return new_guess
...
if __name__ == "__main__":
    # print("ASolver Average Score: " + str(evaluate_solver(AbeSolver, k=1110)))
    print("JPSolver Average Score" + str(evaluate_solver(JPSolver, k=1000)))